...
class Bagel(PreTrainedModel):
    ...
    def __init__(self, language_model, vit_model, config: BagelConfig):
        ...
        if config.visual_gen:
            ...
            if self.use_mae_masking:
                # Learnable placeholder for masked VIT tokens (nonref images).
                # In MAE mode, nonref VIT tokens at masked spatial positions are replaced
                # with this placeholder to prevent information leakage, while noised VAE
                # tokens remain unmasked for flow-matching denoising training.
                self.vit_mask_placeholder = nn.Parameter(torch.zeros(self.hidden_size))
    ...
    def _init_weights(self):
        if self.config.visual_gen:
            ...
            if self.use_mae_masking:
                nn.init.normal_(self.vit_mask_placeholder, std=0.02)
    ...
    def forward(
        ...
        patchified_vit_latent_shapes: Optional[List[Tuple[int, int]]] = None,
        ...
        if self.config.visual_gen:
            ...
            if self.use_masking and packed_vae_types is not None:
                ...
                # === MAE-style Masked Reconstruction Mode ===
                #
                # 设计：所有图片（ref + nonref）都有 noised VAE 和 VIT token。
                #   - VIT token 作为 condition，nonref 的 VIT token 在 masked 位置
                #     被替换为 vit_mask_placeholder，防止信息泄露。
                #   - Noised VAE token 不做 masking，正常参与 flow-matching denoising 训练。
                #   - 所有 VAE token 都算标准 velocity loss（和普通 t2i 一样）。
                #   - 模型需要从 condition（含 masked VIT）中学会补全 masked 区域。
                #
                # Data layout:
                #   先添加所有图片的 VIT token（ref_vit / nonref_vit），
                #   再用 video 模式添加所有 VAE noise token（共享 timestep，帧间双向注意力）。
                #   VIT 和 VAE 通过 packed_vit_types / packed_vae_types 区分 ref/nonref。

                i = random.randint(0, len(self.mask_mode) - 1)
                cur_mode = self.mask_mode[i]
                cur_ratio = self.mask_ratio[i]

                # --- Step 1: 标准 Flow-matching 加噪（所有 VAE token）---
                noise = torch.randn_like(packed_latent_clean)
                ...
                # 所有 VAE token 直接放入 packed_sequence（不做 masking）
                packed_sequence[packed_vae_token_indexes] = packed_latent

                # --- Step 2: 对 nonref 的 VIT token 做 masking ---
                # 直接使用 packed_vit_types 判断每个 VIT image 是否为 nonref。
                if packed_vit_token_indexes is not None and vit_token_seqlens is not None and packed_vit_types is not None:
                    vit_token_offset = 0
                    for vit_img_idx, vit_seqlen in enumerate(vit_token_seqlens):
                        # 通过 packed_vit_types 直接判断当前 VIT 是否为 nonref
                        vit_type = packed_vit_types[vit_token_offset]
                        if vit_type == "nonref_vit":
                            # 直接从数据侧获取 VIT 空间的 (h, w)
                            vit_h, vit_w = patchified_vit_latent_shapes[vit_img_idx]
                            # 直接在 VIT 分辨率上生成 mask
                            mask = generate_connected_masks(
                                B=1, V=1, H=vit_h, W=vit_w,
                                ratio=cur_ratio,
                                device=packed_sequence.device,
                                dtype=torch.uint8,
                                mode=cur_mode,
                            )  # (1, 1, vit_h, vit_w)
                            mask_flat = mask.flatten().bool()[:vit_seqlen]
                            # 获取这些 VIT token 在 packed_sequence 中的索引
                            vit_global_indexes = packed_vit_token_indexes[vit_token_offset:vit_token_offset + vit_seqlen]
        ...
        if self.config.visual_gen:
            if self.use_masking and packed_vae_types is not None:
                ...
                # === MAE-style: 标准 Velocity Loss（所有 VAE token）===
                # 所有 VAE token（ref + nonref）都参与标准 flow-matching velocity loss，
