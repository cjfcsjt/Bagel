...
class Qwen2RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states, tag='None'):
        ...
        # === 检查 rsqrt ===
        has_nan_rsqrt = torch.isnan(rsqrt_val).any().item()
        has_inf_rsqrt = torch.isinf(rsqrt_val).any().item()
        if has_nan_rsqrt or has_inf_rsqrt:
            print(f"      [RMSNorm] rsqrt: NaN={has_nan_rsqrt}, Inf={has_inf_rsqrt}, min={rsqrt_val.min():.6e}, max={rsqrt_val.max():.6e}")
        
        hidden_states = hidden_states * rsqrt_val
        
        # === 检查 weight ===
        has_nan_weight = torch.isnan(self.weight).any().item()
        has_inf_weight = torch.isinf(self.weight).any().item()
        if has_nan_weight or has_inf_weight:
            print(f"      [RMSNorm] Weight: NaN={has_nan_weight}, Inf={has_inf_weight}, min={self.weight.min():.6e}, max={self.weight.max():.6e}")
        ...
        if has_nan_output or has_inf_output:
            print(f"      [RMSNorm] Output: NaN={has_nan_output}, Inf={has_inf_output}, min={output.min():.6e}, max={output.max():.6e}")
