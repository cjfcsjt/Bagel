...
from .modeling_utils import MLPconnector, TimestepEmbedder, PositionEmbedding, PositionEmbedding_Extra, QFormer
...
class G2VLMConfig(PretrainedConfig):
    def __init__(
        ...
        **kwargs
        ...
        self.recon_for_und = recon_for_und
...
class G2VLM(PreTrainedModel):
    ...
    def __init__(self, language_model, vit_model, dino_model, config: G2VLMConfig):
        ...
        # Two-layer cross-attention with 3D RoPE: query x question_text -> updated_query x dino_hidden -> final_query
        self.recon_for_und = getattr(config, 'recon_for_und', False)
        if self.recon_for_und:
            self.num_query_tokens = getattr(config, 'num_query_tokens', 64)
    ...
    def forward(
        ...
        # ===== Pass1 parameters for recon-for-und =====
        pass1_sequence_length: Optional[int] = None,
        pass1_packed_text_ids: Optional[torch.LongTensor] = None,
        pass1_packed_text_indexes: Optional[torch.LongTensor] = None,
        pass1_sample_lens: Optional[List[int]] = None,
        pass1_packed_position_ids: Optional[torch.LongTensor] = None,
        pass1_split_lens: Optional[List[int]] = None,
        pass1_attn_modes: Optional[List[str]] = None,
        pass1_packed_dino_token_indexes: Optional[torch.LongTensor] = None,
        pass1_dino_token_seqlens: Optional[torch.IntTensor] = None,
        pass1_dino_grid_thw: Optional[torch.LongTensor] = None,
        pass1_question_text_indexes: Optional[torch.LongTensor] = None,
        pass1_question_text_lengths: Optional[List[int]] = None,
        pass1_dino_indexes_in_seq: Optional[torch.LongTensor] = None,
        pass1_dino_images_per_sample: Optional[List[int]] = None,
        # ===== Q-Former token indexes for recon-for-und =====
        qformer_token_indexes: Optional[torch.LongTensor] = None,
    ) -> torch.Tensor:
        """
        Args:
        ...
        # 根据token特性准备attention mask, 注意, packing的padding太长了会报错
        if self.config.visual_recon:
        ...
        # ===================================================================
        # Recon-for-Und: Two-pass forward with Q-Former
        # ===================================================================
        if self.config.recon_for_und:
            cu_seqlens = torch.nn.functional.pad(torch.cumsum(pass1_dino_token_seqlens, dim=0), (1, 0)) # dino_token_seqlens：一个 1D 张量，记录每张图像经过 DINO patch 化后的 token 数量。例如 [1369, 1369, 1369] 表示 3 张图各有 1369 个 patch token（即 37×37）。 torch.cumsum(...)：对序列长度做累积求和，得到每张图的结束位置，例如 [1369, 2738, 4107]. Pad: 在最前面补一个 0，变成 [0, 1369, 2738, 4107]。这就是 cumulative sequence lengths（cu_seqlens），是 Flash Attention 的 varlen 接口要求的格式，用来告诉 attention 哪些 token 属于同一张图（避免跨图 attention）
            cu_seqlens = cu_seqlens.to(torch.int32) 
            max_seqlen = torch.max(pass1_dino_token_seqlens).item() # 所有图像中最大的 token 序列长度，也是 Flash Attention 的必要参数
            ...
            # Build pass1 packed_sequence: dino_tokens + system/instruction/question text
            pass1_text_embedding = self.language_model.model.embed_tokens(pass1_packed_text_ids)

            # pass1_sequence_length is already padded to max_pass1_tokens in to_tensor
            pass1_packed_sequence = pass1_text_embedding.new_zeros(
                size=(pass1_sequence_length, self.hidden_size)
            )
            assert pass1_sequence_length == (pass1_packed_dino_token_indexes.shape[0] + pass1_packed_text_indexes.shape[0])
            # Fill dino token embeddings into pass1 sequence
            pass1_packed_sequence[pass1_packed_dino_token_indexes] = packed_dino_token_embed
            pass1_packed_sequence[pass1_packed_text_indexes] = pass1_text_embedding

            # Build pass1 attention mask (sample_lens/split_lens/attn_modes/position_ids already padded in to_tensor)
            pass1_sparse_mask = create_sparse_mask(
                pass1_sample_lens, pass1_split_lens, pass1_attn_modes,
                pass1_text_embedding.device
            )
            pass1_seqlen = sum(pass1_sample_lens)
            pass1_block_mask = create_block_mask(
                pass1_sparse_mask, B=1, H=self.num_heads,
                Q_LEN=pass1_seqlen, KV_LEN=pass1_seqlen,
                device=pass1_text_embedding.device, BLOCK_SIZE=128, _compile=True
            )

            # Build extra_inputs for pass1 (MOE routing)
            pass1_extra_inputs = {}
            if self.use_moe:
                pass1_packed_und_token_indexes = pass1_packed_text_indexes
                pass1_extra_inputs.update(
                    packed_und_token_indexes=pass1_packed_und_token_indexes,
                    packed_geo_token_indexes=pass1_packed_dino_token_indexes,
                )

            # Frozen forward: no gradient through language_model
            with torch.no_grad():
                pass1_output = self.language_model(
                    packed_sequence=pass1_packed_sequence,
                    sample_lens=pass1_sample_lens,
                    attention_mask=pass1_block_mask,
                    packed_position_ids=pass1_packed_position_ids,
                    output_hidden_states=False,
                    **pass1_extra_inputs,
                )
            pass1_hidden = pass1_output.packed_query_sequence  # (pass1_seq_len, D)

            # Extract dino hidden states and question text hidden states from pass1
            dino_hidden_from_pass1 = pass1_hidden[pass1_packed_dino_token_indexes].detach()
            question_hidden_from_pass1 = pass1_hidden[pass1_question_text_indexes].detach()

            # ----- Q-Former: compress dino+question info into query tokens -----
            # For static images, second_per_grid_ts = 1.0 for each image
            num_dino_images = pass1_dino_grid_thw.shape[0]
            second_per_grid_ts = torch.ones(num_dino_images, device=dino_hidden_from_pass1.device)

            # Determine the actual number of samples from qformer_token_indexes
            num_samples = qformer_token_indexes.shape[0] // self.num_query_tokens

            final_query_tokens = self.qformer(
                question_text_hidden=question_hidden_from_pass1,
                dino_hidden=dino_hidden_from_pass1,
                text_lengths=pass1_question_text_lengths, # per sample indicator
                dino_images_per_sample=pass1_dino_images_per_sample, # per sample indicator
                dino_grid_thw=pass1_dino_grid_thw, # per image indicator, for pos and attention
                second_per_grid_ts=second_per_grid_ts, # 
                num_samples=num_samples,
            )  # (num_samples * Nq, D)

            # ----- Fill qformer tokens into the main packed_sequence -----
            # The main packed_sequence already has text embeddings, vit embeddings, and attention mask
            # built from the default parameters (which now carry pass2 data).
            # We just need to fill in the qformer output tokens at their designated positions.
            packed_sequence[qformer_token_indexes] = final_query_tokens

            # Build extra_inputs for pass2 (MOE routing)
            pass2_extra_inputs = {}
            if self.use_moe:
                pass2_packed_und_token_indexes = packed_text_indexes
                if packed_vit_token_indexes is not None:
                    pass2_packed_und_token_indexes = torch.cat([packed_text_indexes, packed_vit_token_indexes], dim=0)
                if qformer_token_indexes is not None:
                    pass2_packed_und_token_indexes = torch.cat([pass2_packed_und_token_indexes, qformer_token_indexes], dim=0)
                pass2_extra_inputs.update(
                    packed_und_token_indexes=pass2_packed_und_token_indexes,
                    packed_geo_token_indexes=torch.tensor([], dtype=torch.long, device=packed_text_indexes.device),
                )

            # Trainable forward on the main packed_sequence (which IS the pass2 sequence)
            ...
            last_hidden_state = last_hidden_state.packed_query_sequence  # (seq_len, D)
            ...
            if ce_loss_indexes is not None and len(ce_loss_indexes) > 0:
                ...
                if _rank == 0 and self._train_check_counter % 50 == 1:
                    with torch.no_grad():
                        ...
                        if pass1_dino_token_seqlens is not None:
                            print(f"\n--- DINO Info ---")
                            print(f"  dino_token_seqlens: {pass1_dino_token_seqlens.tolist()}")
                            print(f"  packed_dino_token_indexes count: {len(pass1_packed_dino_token_indexes)}")
                            print(f"  packed_dino_token_indexes (first 20): {pass1_packed_dino_token_indexes[:20].tolist()}")
            ...
            return EasyDict(ce=ce, ssl_loss=None), {}

        # ===================================================================
        # Main LLM forward (standard path, not recon-for-und)
        # ===================================================================
        extra_inputs = {}
        if self.use_moe:
    ...
    def prepare_dino_images_pi3 (self, curr_kvlens, curr_rope, images, transforms, new_token_ids):
        ...
        for image in images:
            ...
            dino_grid_thw_list.append(dino_image_thw)
        ...
        generation_input = {
            ...
            "packed_key_value_indexes": torch.tensor(packed_key_value_indexes, dtype=torch.long),
            "key_values_lens": torch.tensor(curr_kvlens, dtype=torch.int),
            "dino_grid_thw": torch.stack(dino_grid_thw_list, dim=0) if dino_grid_thw_list else None,
        }
    ...
    def forward_cache_update_dino(
        ...
        dino_grid_thw: torch.Tensor = None,  # unused here, accepted for compatibility
    ):
        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
        packed_sequence = packed_text_embedding.new_zeros((sum(packed_seqlens), self.hidden_size))
        packed_sequence[packed_text_indexes] = packed_text_embedding
    ...
    @torch.no_grad()
    def forward_cache_update_dino_qformer(
        self,
        past_key_values: NaiveCache,
        packed_text_ids: torch.LongTensor,
        packed_text_indexes: torch.LongTensor,
        ...
        question_text_ids: torch.LongTensor,
        system_text_ids: torch.LongTensor = None,
        instruction_text_ids: torch.LongTensor = None,
        curr_rope: int = 0,
    ):
        """
        Two-pass forward for recon-for-und inference:
        Pass 1: [system_text] + [instruction_text] + [dino_images] + [question_text]
                -> full LLM forward with block mask (geo branch)
                -> extract dino_hidden and question_text_hidden
        QFormer: question_text_hidden + dino_hidden -> query_tokens
        Pass 2: Write query_tokens into main KV cache as und-mode tokens

        Note: Pass 1 uses forward_train (full forward with attention mask), NOT forward_inference
        (KV cache mode), because pass1 is a standalone forward that does not interact with the
        main KV cache. This matches the training-side behavior exactly.

        Args:
            question_text_ids: tokenized question text ids (1D LongTensor)
            dino_grid_thw: (num_images, 3) grid dimensions for dino images
            (other args same as forward_cache_update_dino)
        """
        ...
        B_dino, P, D_dino = packed_dino_token_embed.size()
        packed_dino_token_embed = packed_dino_token_embed.reshape(B_dino * P, D_dino)
        packed_dino_token_embed = self.dino2llm(packed_dino_token_embed)

        # ===== Pass 1: Full forward (not KV cache) with block mask =====
        # Build pass1 sequence matching training-side structure:
        #   [system_text] [instruction_text] [<soi> dino_tokens <eoi>] × N + [question_text]
        # All position IDs start from 0 (independent forward, not from main KV cache)

        # --- Prepare prefix text: system + instruction ---
        prefix_text_ids_list = []
        if system_text_ids is not None and system_text_ids.numel() > 0:
            prefix_text_ids_list.append(system_text_ids)
        if instruction_text_ids is not None and instruction_text_ids.numel() > 0:
            prefix_text_ids_list.append(instruction_text_ids)
        if prefix_text_ids_list:
            prefix_text_ids = torch.cat(prefix_text_ids_list, dim=0)
        else:
            prefix_text_ids = torch.tensor([], dtype=torch.long, device=device)
        prefix_len = prefix_text_ids.shape[0]

        if prefix_len > 0:
            prefix_text_embedding = self.language_model.model.embed_tokens(prefix_text_ids)

        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
        dino_seq_len = packed_seqlens.sum().item()

        question_text_embedding = self.language_model.model.embed_tokens(question_text_ids)
        question_text_len = question_text_ids.shape[0]

        # Layout: [prefix_text | dino_seq | question_text]
        pass1_total_len = prefix_len + dino_seq_len + question_text_len
        pass1_packed_sequence = packed_text_embedding.new_zeros((pass1_total_len, self.hidden_size))

        # Fill prefix text
        pass1_prefix_indexes = torch.arange(0, prefix_len, device=device, dtype=torch.long)
        if prefix_len > 0:
            pass1_packed_sequence[pass1_prefix_indexes] = prefix_text_embedding

        # Fill DINO sequence (shift indexes by prefix_len)
        shifted_text_indexes = packed_text_indexes + prefix_len
        shifted_dino_token_indexes = packed_dino_token_indexes + prefix_len

        if packed_dino_token_embed.dtype != pass1_packed_sequence.dtype:
            packed_dino_token_embed = packed_dino_token_embed.to(pass1_packed_sequence.dtype)
        pass1_packed_sequence[shifted_text_indexes] = packed_text_embedding
        pass1_packed_sequence[shifted_dino_token_indexes] = packed_dino_token_embed

        # Fill question text
        pass1_question_text_indexes = torch.arange(
            prefix_len + dino_seq_len,
            prefix_len + dino_seq_len + question_text_len,
            device=device, dtype=torch.long,
        )
        pass1_packed_sequence[pass1_question_text_indexes] = question_text_embedding

        # --- Build pass1 position ids (from 0, independent of main KV cache) ---
        curr_rope_id = 0
        pass1_position_ids_parts = []

        # Prefix text position ids
        if prefix_len > 0:
            prefix_pos_ids = torch.arange(
                curr_rope_id, curr_rope_id + prefix_len, device=device, dtype=torch.long
            ).expand(3, -1)
            pass1_position_ids_parts.append(prefix_pos_ids)
            curr_rope_id += prefix_len

        # DINO sequence position ids (re-compute from 0)
        for i, dino_seqlen in enumerate(dino_token_seqlens.tolist()):
            soi_pos = torch.full((1,), curr_rope_id, dtype=torch.long, device=device)
            pass1_position_ids_parts.append(soi_pos.expand(3, 1))
            curr_rope_id += 1

            dino_thw_i = dino_grid_thw[i]
            dino_pos_ids, rope_deltas = get_rope_index_image_3D_dino(
                dino_thw_i, curr_rope_id, device=device
            )
            pass1_position_ids_parts.append(dino_pos_ids)
            curr_rope_id += rope_deltas + 1

            eoi_pos = torch.full((1,), curr_rope_id, dtype=torch.long, device=device)
            pass1_position_ids_parts.append(eoi_pos.expand(3, 1))
            curr_rope_id += 1

        # Question text position ids
        if question_text_len > 0:
            question_pos_ids = torch.arange(
                curr_rope_id, curr_rope_id + question_text_len, device=device, dtype=torch.long
            ).expand(3, -1)
            pass1_position_ids_parts.append(question_pos_ids)
            curr_rope_id += question_text_len

        pass1_position_ids = torch.cat(pass1_position_ids_parts, dim=1)

        # ---- Build attention mask (split_lens + attn_modes) ----
        # Matches training-side behavior in dataset_base.py _pack_pass1_sequence:
        #   - Each text item (system, instruction, question) -> independent causal split
        #   - Each DINO image (<soi> + dino_tokens + <eoi>) -> independent full split
        # IMPORTANT: sum(split_lens) must == pass1_total_len, otherwise create_sparse_mask
        # will create tensors shorter than Q_LEN, causing index-out-of-bounds in block_mask.
        pass1_split_lens = []
        pass1_attn_modes = []

        # Prefix text splits (system + instruction)
        if system_text_ids is not None and system_text_ids.numel() > 0:
            pass1_split_lens.append(system_text_ids.shape[0])
            pass1_attn_modes.append('causal')
        if instruction_text_ids is not None and instruction_text_ids.numel() > 0:
            pass1_split_lens.append(instruction_text_ids.shape[0])
            pass1_attn_modes.append('causal')

        # DINO image splits
        for dino_seqlen in dino_token_seqlens.tolist():
            # Each image: 1(start_of_image) + dino_tokens + 1(end_of_image) = dino_seqlen + 2
            pass1_split_lens.append(dino_seqlen + 2)
            pass1_attn_modes.append('full')

        # Question text split
        if question_text_len > 0:
            pass1_split_lens.append(question_text_len)
            pass1_attn_modes.append('causal')

        pass1_sample_lens = [pass1_total_len]  # single sample for inference
        ...
        pass1_block_mask = create_block_mask(
            pass1_sparse_mask, B=1, H=self.num_heads,
            Q_LEN=pass1_total_len, KV_LEN=pass1_total_len,
            device=device, BLOCK_SIZE=128, _compile=True
        )

        # Build MoE routing for pass1 (use shifted indexes since prefix was prepended)
        pass1_extra_inputs = {}
        if self.use_moe:
            # All text tokens (prefix + soi/eoi markers + question) go to und branch
            all_text_parts = []
            if prefix_len > 0:
                all_text_parts.append(pass1_prefix_indexes)
            all_text_parts.append(shifted_text_indexes)
            if question_text_len > 0:
                all_text_parts.append(pass1_question_text_indexes)
            pass1_all_text_indexes = torch.cat(all_text_parts, dim=0)
            pass1_extra_inputs = {
                "packed_geo_token_indexes": shifted_dino_token_indexes,
                "packed_und_token_indexes": pass1_all_text_indexes,
            }

        # Frozen full forward (forward_train): standalone, no KV cache interaction.
        # We call forward_train directly. The forward() dispatcher in Qwen2VLModel
        # and Qwen2VLMoTDecoderLayer detects 'packed_sequence' kwarg and routes to
        # forward_train even in eval mode.
        # Disable autocast completely so all ops use the model's native parameter dtype
        # uniformly, avoiding dtype mismatches between new_zeros containers and
        # layernorm/linear outputs.
        with torch.autocast(device_type='cuda', enabled=False):
            param_dtype = next(self.language_model.parameters()).dtype
            pass1_packed_sequence = pass1_packed_sequence.to(param_dtype)
            pass1_output = self.language_model.forward_train(
                packed_sequence=pass1_packed_sequence,
                sample_lens=pass1_sample_lens,
                attention_mask=pass1_block_mask,
                packed_position_ids=pass1_position_ids,
                output_hidden_states=False,
                **pass1_extra_inputs,
            )
        pass1_hidden = pass1_output.packed_query_sequence  # (pass1_total_len, D)

        # Extract dino hidden and question text hidden from pass1
        # Use shifted indexes since prefix text was prepended to the sequence
        dino_hidden_from_pass1 = pass1_hidden[shifted_dino_token_indexes].detach()
        question_hidden_from_pass1 = pass1_hidden[pass1_question_text_indexes].detach()

        # ===== QFormer: compress dino + question info into query tokens =====
        num_dino_images = dino_grid_thw.shape[0]
        second_per_grid_ts = torch.ones(num_dino_images, device=device)
        num_samples = 1  # inference is always batch=1

        final_query_tokens = self.qformer(
            question_text_hidden=question_hidden_from_pass1,
            dino_hidden=dino_hidden_from_pass1,
            text_lengths=[question_text_len],
            dino_images_per_sample=[num_dino_images],
            dino_grid_thw=dino_grid_thw,
            second_per_grid_ts=second_per_grid_ts,
            num_samples=num_samples,
        )  # (num_samples * Nq, D) = (Nq, D)

        # ===== Pass 2: Write QFormer query tokens into main KV cache =====
        # Treat query tokens as "und" mode tokens (like VIT tokens)
        nq = final_query_tokens.shape[0]

        # Build position ids for query tokens (continue from main KV cache rope position)
        query_pos_ids = torch.arange(
            curr_rope, curr_rope + nq, device=device, dtype=torch.long
        ).expand(3, -1)

        query_seqlens = torch.tensor([nq], dtype=torch.int, device=device)
        query_indexes = torch.arange(
            key_values_lens.sum().item(),
            key_values_lens.sum().item() + nq,
            device=device, dtype=torch.long,
        )
        ...
        output = self.language_model.forward_inference(
            packed_query_sequence=final_query_tokens,
            query_lens=query_seqlens,
            packed_query_position_ids=query_pos_ids,
        ...
        return past_key_values, last_hidden_state, nq
