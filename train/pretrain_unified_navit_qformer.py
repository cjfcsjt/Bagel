...
from modeling.g2vlm.g2vlm_qformer import G2VLMConfig, G2VLM
...
from modeling.qwen2vl.modeling_qwen2_vl import Qwen2VLForConditionalGeneration, Qwen2VisionTransformerPretrainedModel
...
class TrainingArguments:
    ...
    recon_for_und: bool = field(
        default=False,
        metadata={"help": "Enable recon-for-und mode with Q-Former bridging DINO and LLM."}
    )
    num_query_tokens: int = field(
        default=64,
        metadata={"help": "Number of Q-Former query tokens for recon-for-und."}
    )
    qformer_num_heads: int = field(
        default=16,
        metadata={"help": "Number of attention heads in Q-Former for recon-for-und."}
    )
...
def main():
    ...
    if training_args.finetune_from_hf:
        llm_config = Qwen2VLConfig.from_json_file(os.path.join(model_args.model_path, "text_config.json")) # 从本地json文件直接加载配置，因为可能已经包含了Bagel的特有配置
    else:
    ...
    llm_config.freeze_recon = training_args.freeze_recon # 是否冻结视觉理解（understanding）分支的连接器
    ...
    config = G2VLMConfig(
        ...
        recon_for_und=training_args.recon_for_und,
        num_query_tokens=training_args.num_query_tokens,
        qformer_num_heads=training_args.qformer_num_heads,
    )
