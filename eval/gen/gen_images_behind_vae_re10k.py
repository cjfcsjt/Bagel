...
# -*- coding: utf-8 -*-
"""
Behind-VAE 模式的多视角生成评估脚本（RE10K 数据集）。

behind_vae 模式下的推理流程（方案A：仅 ref VIT condition）：
1. 注入文本 token（'Reconstruct the scene' 模板）
2. 仅注入 ref 图片的 VIT token 作为 condition（不注入 nonref VIT，避免信息泄露）
3. 通过 flow-matching denoising 从纯噪声生成 nonref VAE latent
4. 解码 VAE latent 为图片

用法:
    torchrun --nproc_per_node=8 eval/gen/gen_images_behind_vae_re10k.py \
        --model-path /path/to/model \
        --output_dir ./output/behind_vae_re10k \
...
def sample_indices(num_imgs, max_frames, max_distance=10):
    ...
    与训练时 edit_recon_dataset.py 中的 _sample_indices 逻辑一致。
    """
...
def generate_nonref_images_behind_vae(
    ...
    """
    Behind-VAE 模式（方案A）：给定 ref 图片的 VIT condition，生成 nonref 图片。

    推理流程（方案A：仅 ref VIT condition，避免 nonref VIT 信息泄露）：
    1. 注入文本 token（'Reconstruct the scene' 模板，通过 prepare_prompts）
    2. 仅注入 ref 图片的 VIT token（不注入 nonref VIT，避免信息泄露）
    3. 对每张 nonref 图片，准备 VAE latent 并通过 flow-matching denoise 生成

    注意：behind_vae 模式下 ref 没有 VAE token，只有 VIT。
    方案A 与训练时的区别：训练时 nonref 的未被 mask 的 VIT patch 对 VAE 可见，
    但推理时完全不注入 nonref VIT，这是一种更保守的策略。

    Args:
        ref_images: list of PIL.Image, ref 视角图片
        nonref_images: list of PIL.Image, 待生成的 nonref 视角图片（仅用于确定输出尺寸）
        其他参数: 模型和生成超参数
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        # ============================================================
        # Step 1: 注入文本 token（与训练时 apply_template_qwenvl2 一致）
        # ============================================================
        ...
        # 构造与训练时一致的文本 prompt
        # 训练时 question = 'Reconstruct the scene' + '<vit_image>' * N + '<vae_image>' * nonref_count
        # 推理时只注入文本部分，VIT/VAE 通过 prepare_vit_images/prepare_vae_latent 注入
        ...
        # ============================================================
        # Step 2: 仅注入 ref 图片的 VIT token（方案A：不注入 nonref VIT）
        # ============================================================
        ...
        for nonref_img in nonref_images:
            ...
            # --- cfg_text: 去掉文本 prompt 后的 condition（只保留 VIT token）---
            ...
            # --- cfg_img: 完全空的 condition（无任何 token）---
            ...
            # --- 主 context: 文本 + VIT condition + 生成 nonref VAE latent ---
...
def main():
    parser = argparse.ArgumentParser(description="Behind-VAE multi-view generation on RE10K.")
    ...
    # VAE transform: 与训练时一致
    vae_transform = ImageTransform(1024, 512, 16)
    # VIT transform: 与训练时一致（behind_vae 模式使用较小的 VIT 分辨率）
