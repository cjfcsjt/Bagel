...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def to_tensor(self, sequence_status):
        ...
        # Propagate per-VAE-token type labels (ref / nonref_clean / nonref_noise / None)
        if len(sequence_status['packed_vae_types']) > 0:
            data['packed_vae_types'] = sequence_status['packed_vae_types']
