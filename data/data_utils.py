...
def create_sparse_mask(document_lens, split_lens, attn_modes, device):
    def causal_mask(b, h, q_idx, kv_idx): # 因果掩码
        return q_idx >= kv_idx # 标准自回归：只能看到自己和左边的 token。

    def full_and_noise_mask(b, h, q_idx, kv_idx): # 同段双向掩码
        return (full_and_noise_seq_id[q_idx] == full_and_noise_seq_id[kv_idx]) & (full_and_noise_seq_id[q_idx] >= 0) # 如果 q 和 kv 属于同一个 full 或 noise split（id 相同且 ≥ 0），则允许双向注意。causal split 的 id 被设为 -1，不会命中这个条件。

    def remove_noise_mask(b, h, q_idx, kv_idx): # 隔离不同 noise 段
        return (~((noise_seq_id[kv_idx] >= 0) & (noise_seq_id[q_idx] != noise_seq_id[kv_idx]))) # 如果 kv 是 noise token（noise_seq_id[kv_idx] >= 0），且 q 不属于同一个 noise 段 → 禁止。
 
    def sample_mask(b, h, q_idx, kv_idx): # 样本隔离
        return document_id[q_idx] == document_id[kv_idx] # packed batch 中不同样本之间完全隔离，防止跨样本信息泄漏。
